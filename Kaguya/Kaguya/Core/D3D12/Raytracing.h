...
{
	RaytracingMemoryPage*	  Parent	 = nullptr;
	UINT64					  Size		 = 0;
	UINT64					  Offset	 = 0;
	UINT64					  UnusedSize = 0;
...
class RaytracingMemoryPage : public DeviceChild
{
public:
	RaytracingMemoryPage() noexcept = default;

	RaytracingMemoryPage(Device* Parent, UINT64 PageSize)
		: DeviceChild(Parent)
		, PageSize(PageSize)
	{
	}
...
class RaytracingMemoryAllocator : public DeviceChild
{
public:
	RaytracingMemoryAllocator() noexcept = default;

	RaytracingMemoryAllocator(
		Device*				  Parent,
	...
	RaytracingMemorySection Allocate(UINT64 SizeInBytes)
	{
		...
		if (AlignedSizeInBytes > DefaultPageSize)
		{
			...
			RaytracingMemoryPage* Page = Pages.back().get();
			Section.Parent			   = Page;
		...
		else
		{
			...
			{
				RaytracingMemoryPage* Page = Pages[i].get();
...
private:
	...
	bool FindFreeSubBlock(RaytracingMemoryPage* Page, RaytracingMemorySection* OutMemorySection, UINT64 SizeInBytes)
	{
	...
	std::vector<std::unique_ptr<RaytracingMemoryPage>> Pages;
};
...
class RaytracingAccelerationStructureManager : public DeviceChild
{
public:
	RaytracingAccelerationStructureManager() noexcept = default;

	RaytracingAccelerationStructureManager(Device* Parent, UINT64 PageSize)
		: DeviceChild(Parent)
		, PageSize(PageSize)
		, ScratchPool(
			  Parent,
	...
	void SetSyncPoint(UINT64 AccelerationStructureIndex, CommandSyncPoint SyncPoint)
	{
...
private:
	UINT64 GetAccelerationStructureIndex()
	{
		...
		if (!IndexQueue.empty())
		{
			...
			AccelerationStructures[NewIndex] = std::make_unique<AccelerationStructure>();
			IndexQueue.pop();
		}
		else
		{
			AccelerationStructures.push_back(std::make_unique<AccelerationStructure>());
			NewIndex = Index++;
		}
		return NewIndex;
	}
