...
void DynamicResourceDescriptorHeap::Initialize(UINT NumDescriptors, bool ShaderVisible, D3D12_DESCRIPTOR_HEAP_TYPE Type)
{
...
void DynamicResourceDescriptorHeap::Allocate(
...
void DynamicResourceDescriptorHeap::Release(UINT Index)
{
	std::scoped_lock _(Mutex);
...
D3D12_CPU_DESCRIPTOR_HANDLE DynamicResourceDescriptorHeap::hCPU(UINT Index) const noexcept
{
	D3D12_CPU_DESCRIPTOR_HANDLE hCPU = {};
...
D3D12_GPU_DESCRIPTOR_HANDLE DynamicResourceDescriptorHeap::hGPU(UINT Index) const noexcept
{
	D3D12_GPU_DESCRIPTOR_HANDLE hGPU = {};
...
void DescriptorPage::Initialize(UINT NumDescriptors, D3D12_DESCRIPTOR_HEAP_TYPE Type)
{
...
std::optional<DescriptorArray> DescriptorPage::Allocate(UINT NumDescriptors)
{
	...
	return DescriptorArray(this, Handle, Offset, NumDescriptors);
}

void DescriptorPage::Release(DescriptorArray&& DescriptorArray)
{
	FreeBlock(DescriptorArray.GetOffset(), DescriptorArray.GetNumDescriptors());
}

void DescriptorPage::AllocateFreeBlock(TOffset Offset, TSize Size)
{
...
void DescriptorPage::FreeBlock(TOffset Offset, TSize Size)
{
...
DescriptorArray DescriptorAllocator::Allocate(UINT NumDescriptors)
{
	DescriptorArray DescriptorArray;
	...
	{
		auto NewPage = std::make_unique<DescriptorPage>(GetParentDevice());
...
void DynamicDescriptorHeap::Initialize(UINT NumDescriptors, D3D12_DESCRIPTOR_HEAP_TYPE Type)
{
	Desc = { .Type			 = Type,
			 .NumDescriptors = NumDescriptors,
...
UINT DescriptorHandleCache::CommitDescriptors(
