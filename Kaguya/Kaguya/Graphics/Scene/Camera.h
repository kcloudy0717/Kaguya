...
struct Camera
{
	...
	{
		return Transform == Camera.Transform && FoVY == Camera.FoVY && NearZ == Camera.NearZ && FarZ == Camera.FarZ;
	}

	bool operator!=(const Camera& Camera) const { return !(*this == Camera); }

	float static CalculateFoVX(int w, int h, float FoVY) { return 2.0f * atan(tan(FoVY * 0.5f) * float(w) / float(h)); }
	float static CalculateFoVY(int w, int h, float FoVX) { return 2.0f * atan(tan(FoVX * 0.5f) * float(h) / float(w)); }
