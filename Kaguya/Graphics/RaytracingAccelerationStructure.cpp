	...
	UINT NumHitGroups)
{
	...
	m_InstanceDescs = RenderDevice.CreateBuffer(&allocationDesc, sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * Scene::MAX_INSTANCE_SUPPORTED, D3D12_RESOURCE_FLAG_NONE, 0, D3D12_RESOURCE_STATE_GENERIC_READ);
...
void RaytracingAccelerationStructure::Build(CommandList& CommandList)
{
	...
	{
		// TLAS Scratch
		m_TLASScratch = RenderDevice.CreateBuffer(&allocationDesc, scratchSize, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS, 0, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
	}

	if (!m_TLASResult || m_TLASResult->pResource->GetDesc().Width < resultSize)
	{
		// TLAS Result
		m_TLASResult = RenderDevice.CreateBuffer(&allocationDesc, resultSize, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS, 0, D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE);
	}
	...
	m_TopLevelAccelerationStructure.Generate(CommandList, m_TLASScratch->pResource.Get(), m_TLASResult->pResource.Get(), m_InstanceDescs->pResource->GetGPUVirtualAddress());
}
