...
CommandList::CommandList(_In_ ID3D12Device4* pDevice, _In_ D3D12_COMMAND_LIST_TYPE Type)
{
	/*
	...
	ThrowIfFailed(pDevice->CreateCommandList1(
		...
		Type,
		D3D12_COMMAND_LIST_FLAG_NONE,
		IID_PPV_ARGS(m_pPendingCommandList.ReleaseAndGetAddressOf())));
	// Microsoft::WRL::ComPtr<ID3D12CommandAllocator> Alloc;
	// pDevice->CreateCommandAllocator(Type, IID_PPV_ARGS(&Alloc));
	// ThrowIfFailed(pDevice->CreateCommandList(1, Type, Alloc.Get(), nullptr,
	// IID_PPV_ARGS(pCommandList.ReleaseAndGetAddressOf()))); pCommandList->Close();

	// pDevice->CreateCommandAllocator(Type, IID_PPV_ARGS(&Alloc));
	// ThrowIfFailed(pDevice->CreateCommandList(1, Type, Alloc.Get(), nullptr,
	// IID_PPV_ARGS(pPendingCommandList.ReleaseAndGetAddressOf()))); pPendingCommandList->Close();
}

bool CommandList::Close(_In_ ResourceStateTracker* pGlobalResourceStateTracker)
{
	FlushResourceBarriers();
	ThrowIfFailed(m_pCommandList->Close());

	UINT numPendingBarriers =
		m_ResourceStateTracker.FlushPendingResourceBarriers(*pGlobalResourceStateTracker, m_pPendingCommandList.Get());
	pGlobalResourceStateTracker->UpdateResourceStates(m_ResourceStateTracker);
	ThrowIfFailed(m_pPendingCommandList->Close());
	return numPendingBarriers > 0;
}

void CommandList::Reset(_In_ UINT64 FenceValue, _In_ UINT64 CompletedValue, _In_ CommandQueue* pCommandQueue)
{
...
void CommandList::SetDescriptorHeaps(_In_ const ResourceViewHeaps& ResourceViewHeaps)
{
	...
	_In_ UINT				   Subresource /*= D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES*/)
{
	...
	// The D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES flag is used to transition all subresources in a resource at the same
	// time.
	m_ResourceStateTracker.ResourceBarrier(
		CD3DX12_RESOURCE_BARRIER::Transition(pResource, D3D12_RESOURCE_STATE_UNKNOWN, TransitionState, Subresource));
}

void CommandList::AliasingBarrier(_In_ ID3D12Resource* pBeforeResource, _In_ ID3D12Resource* pAfterResource)
{
	// Aliasing barriers indicate a transition between usages of two different resources which have mappings into the
	// same heap. The application can specify both the before and the after resource. Note that one or both resources
	// can be NULL (indicating that any tiled resource could cause aliasing).
	m_ResourceStateTracker.ResourceBarrier(CD3DX12_RESOURCE_BARRIER::Aliasing(pBeforeResource, pAfterResource));
}

void CommandList::UAVBarrier(_In_ ID3D12Resource* pResource)
{
	...
	// The application can specify NULL for the barrier's UAV, which indicates that any UAV access could require the
	// barrier.
...
void CommandList::Dispatch(_In_ UINT ThreadGroupCountX, _In_ UINT ThreadGroupCountY, _In_ UINT ThreadGroupCountZ)
{
	FlushResourceBarriers();
	m_pCommandList->Dispatch(ThreadGroupCountX, ThreadGroupCountY, ThreadGroupCountZ);
}

void CommandList::DispatchRays(_In_ const D3D12_DISPATCH_RAYS_DESC* pDesc)
{
	FlushResourceBarriers();
	m_pCommandList->DispatchRays(pDesc);
}

void CommandList::DispatchMesh(_In_ UINT ThreadGroupCountX, _In_ UINT ThreadGroupCountY, _In_ UINT ThreadGroupCountZ)
{
	FlushResourceBarriers();
