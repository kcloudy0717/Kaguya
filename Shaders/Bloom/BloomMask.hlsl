...
[numthreads(8, 8, 1)] void CSMain(CSParams Params)
{
	...
	float2 uv	  = (Params.DispatchThreadID.xy + 0.5f) * InverseOutputSize;
	float2 offset = InverseOutputSize * 0.25f;

	// Use 4 bilinear samples to guarantee we don't undersample when downsizing by more than 2x
	float3 color1 = Input.SampleLevel(g_SamplerLinearClamp, uv + float2(-offset.x, -offset.y), 0.0f).rgb;
	float3 color2 = Input.SampleLevel(g_SamplerLinearClamp, uv + float2(+offset.x, -offset.y), 0.0f).rgb;
	float3 color3 = Input.SampleLevel(g_SamplerLinearClamp, uv + float2(-offset.x, +offset.y), 0.0f).rgb;
	float3 color4 = Input.SampleLevel(g_SamplerLinearClamp, uv + float2(+offset.x, +offset.y), 0.0f).rgb;

	float luma1 = luminance(color1);
	float luma2 = luminance(color2);
	float luma3 = luminance(color3);
	float luma4 = luminance(color4);
	...
	if (!any(isnan(result)))
	{
		Output[Params.DispatchThreadID.xy] = result;
	}
}